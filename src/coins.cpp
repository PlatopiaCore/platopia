...
void AddCoins(CCoinsViewCache &cache, const CTransaction &tx, int nHeight) {
    ...
    for (size_t i = 0; i < tx.vout.size(); ++i) {
        ...
        // transactions.
        cache.AddCoin(COutPoint(txid, i, tx.vout[i].nValue), Coin(tx.vout[i], nHeight, fCoinbase),
                      fCoinbase);
    }
}
...
double CCoinsViewCache::GetPriority(const CTransaction &tx, int nHeight,
    ...
    for (const CTxIn &txin : tx.vin) {
        ...
        if (int64_t(coin.GetHeight()) <= nHeight) {
            dResult += double(coin.GetTxOut().nValue) *
