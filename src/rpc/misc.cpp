...
static UniValue validateaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given platopia address.\n"
            "\nArguments:\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Platopia address and we have full public key:
...
static UniValue createmultisig(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "are platopia addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) platopia address or hex-encoded public "
            "key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue getmemoryinfo(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "managed\n"
            "    \"locked\": xxxxxx,       (numeric) CAmount of bytes that "
