...
UniValue blockheaderToJSON(const CBlockIndex *blockindex) {
    ...
    result.push_back(Pair("chaininterest", blockindex->nChainInterest));
    result.push_back(Pair("time", int64_t(blockindex->nTime)));
    result.push_back(
        Pair("mediantime", int64_t(blockindex->GetMedianTimePast())));
    result.push_back(Pair("nonce", uint64_t(blockindex->nNonce)));
    result.push_back(Pair("mixhash", ethash_h256_encode(blockindex->hashMix)));
...
UniValue blockToJSON(const Config &config, const CBlock &block,
    ...
    result.push_back(Pair("chaininterest", block.nChainInterest));
    ...
    result.push_back(Pair("mixhash", ethash_h256_encode(block.hashMix)));
...
void entryToJSON(UniValue &info, const CTxMemPoolEntry &e) {
    ...
    info.push_back(
        Pair("descendantfees", e.GetModFeesWithDescendants()));
    info.push_back(Pair("ancestorcount", e.GetCountWithAncestors()));
    info.push_back(Pair("ancestorsize", e.GetSizeWithAncestors()));
    info.push_back(
        Pair("ancestorfees", e.GetModFeesWithAncestors()));
...
UniValue gettxout(const Config &config, const JSONRPCRequest &request) {
    ...
    ret.push_back(Pair("locktime", ValueFromAmount(coin.GetTxOut().nLockTime)));
    ret.push_back(Pair("principal", ValueFromAmount(coin.GetTxOut().nPrincipal)));
    UniValue o(UniValue::VOBJ);
...
UniValue getblockchaininfo(const Config &config,
    ...
    obj.push_back(Pair("chainInterest", chainActive.Tip()->nChainInterest));
