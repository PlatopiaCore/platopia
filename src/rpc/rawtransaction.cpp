...
void TxToJSON(const Config &config, const CTransaction &tx,
    ...
    UniValue arrAddresses(UniValue::VARR);
...
//        }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        out.push_back(Pair("principal", ValueFromAmount(txout.nPrincipal)));
        if((int)txout.strContent.size()>MAX_STANDARD_TX_SIZE)
        ...
        addrs = find_value(o, "addresses");
        if (!addrs.isNull()) {
            for (int i = 0; i < addrs.size(); i++) {
                if (!arrAddresses.exists(addrs[i].getValStr()))
                    arrAddresses.push_back(addrs[i]);
            }
        }
...
static UniValue createrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "[{\"address\":\"address\",\"amount\":x.xx,\"principal\":x.xx,\"locktime\":xxx, \"content\":\"content\"}]\n"
			...
			"     [\n"
			"       {\n"
            "      	  \"address\":\"address\", (string, required) The　platopia address\n"
			"      	  \"amount\":x.xx, (numeric, required) The　amount of PTA\n"
            "      	  \"principal\":x.xx, (numeric, optional) The principal locking for interest\n"
			"      	  \"locktime\":locktime, (numeric, optional) The　number of blocks locked\n"
            ...
            HelpExampleCli("createrawtransaction",
                           "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" "
                           "\"[{\\\"address\\\":\\\"address\\\", \\\"amount\\\":1.23}]\"") +
            HelpExampleCli("createrawtransaction",
                           "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" "
                           "\"[{\\\"address\\\":\\\"address\\\", \\\"amount\\\":1.23, "
                           "\\\"principal\\\":1.22, \\\"locktime\\\":15360, "
                           "\\\"content\\\":\\\"hex content\\\"}]\"") +
    ...
    for (size_t idx = 0; idx < inputs.size(); idx++) {
        ...
        CTxIn in(COutPoint(txid, nOutput, coin.GetTxOut().nValue), CScript());
    ...
    for (size_t idx = 0; idx < outputs.size(); idx++) {
		...
		const UniValue &address_v = find_value(o, "address");
		if (!address_v.isStr()) {
			throw JSONRPCError(RPC_INVALID_PARAMETER,
							   "Invalid parameter, missing address key");
		}
		string address(address_v.getValStr());
		...
		if (!amount_v.isNum()) {
			throw JSONRPCError(RPC_INVALID_PARAMETER,
							   "Invalid parameter, missing satoshi key");
		}
		CAmount satoshi = AmountFromValue(amount_v);
		if(satoshi <= 0 || satoshi > MAX_MONEY)
		{
			throw JSONRPCError(RPC_INVALID_PARAMETER, string("output satoshi out of range"));
		}

		CAmount principal = 0;
		const UniValue &principal_v = find_value(o, "principal");
		if(!principal_v.isNull())
		{
			if (!principal_v.isNum()) {
				throw JSONRPCError(RPC_INVALID_PARAMETER,
								   "Invalid parameter, output principal must be integer");
			}
			principal = AmountFromValue(principal_v);
			if(principal < 0 || principal > MAX_MONEY)
			{
				throw JSONRPCError(RPC_INVALID_PARAMETER, string("output principal out of range"));
			}
		}
		...
		if(locktime_v.isNull())
		{
			if(principal != 0)
			{
				throw JSONRPCError(RPC_INVALID_PARAMETER, string("Invalid parameter, output having principal but missing locktime key"));
			}
		}
		else
		{
			if (!locktime_v.isNum()) {
				throw JSONRPCError(RPC_INVALID_PARAMETER,
								   "Invalid parameter, missing locktime key");
			}
			locktime = locktime_v.get_int();
			if(locktime <= 0)
			{
				throw JSONRPCError(RPC_INVALID_PARAMETER, string("output locktime must > 0"));
			}
		}

		string content;
		const UniValue &content_v = find_value(o, "content");
		if(content_v.isStr() && content_v.getValStr().size() > 0)
		{
			std::vector<uint8_t> vContent = ParseHexV(content_v, "content");
			for (std::vector<uint8_t>::iterator iter = vContent.begin(); iter != vContent.end(); ++iter) {
				content += *iter;
			}
		}

		CScript scriptPubKey = GetScriptForDestination(destination);
		CTxOut out(satoshi, scriptPubKey, content, locktime, principal);
...
static UniValue decoderawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "       \"n\" : n,                    (numeric) index\n"
            "       \"principal\" : x.xxx,    (numeric) The principal in " + CURRENCY_UNIT + "\n"
            "       \"locktime\" : locktime,　　　　　　(numeric) The locktime\n"
            "       \"content\" : \"hex content\",              (string) content\n"
            "       \"scriptPubKey\" : {          (json object)\n"
            "         \"asm\" : \"asm\",          (string) the asm\n"
            "         \"hex\" : \"hex\",          (string) the hex\n"
            "         \"reqSigs\" : n,            (numeric) The required sigs\n"
...
static UniValue signrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "\"redeemScript\":\"hex\",\"amount\":x.xx},...] [\"privatekey1\",...] sighashtype "
            ")\n"
    ...
    for (size_t i = 0; i < mergedTx.vin.size(); i++) {
        ...
        for (const CMutableTransaction &txv : txVariants) {
            if (txv.vin.size() > i) {
                sigdata = CombineSignatures(
                    prevPubKey,
                    TransactionSignatureChecker(&txConst, i), sigdata,
        ...
        if (!VerifyScript(
                ...
                TransactionSignatureChecker(&txConst, i), &serror)) {
...
static UniValue sendrawtransaction(const Config &config,
    ...
    if (request.params.size() > 1 && request.params[1].get_bool()) {
        nMaxRawTxFee = CAmount(0);
    }

    CCoinsViewCache &view = *pcoinsTip;
    bool fHaveChain = false;
    for (size_t o = 0; !fHaveChain && o < tx->vout.size(); o++) {
        const Coin &existingCoin = view.AccessCoin(COutPoint(txid, o, tx->vout[o].nValue));
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        if (!AcceptToMemoryPool(config, mempool, state, std::move(tx),
                                true, &fMissingInputs, nullptr, false,
                                nMaxRawTxFee)) {
            if (state.IsInvalid()) {
