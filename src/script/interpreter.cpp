...
public:
    ...
    void Serialize(S &s) const {
        // Serialize nVersion
    	::Serialize(s, VARINT(txTo.nVersion));
        // Serialize nFlags
        ::Serialize(s, VARINT(txTo.nFlags));
        // Serialize vin
...
bool CheckCompactSig(const uint256 hash,const vector<unsigned char> vchSig,const CKeyID keyID)
{
    CPubKey pubkey;
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if ((flags & SCRIPT_VERIFY_P2SH) && scriptPubKey.IsPayToScriptHash())
    {
        ...
        if (!EvalScript(stack, pubKey2, flags, checker, serror)) // Check inside script, the whole script as script + scriptPubkey
            // serror is set
            return false;
        if (stack.empty())
            return set_error(serror, SCRIPT_ERR_EVAL_FALSE);
