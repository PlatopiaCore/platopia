    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            if (insecure_rand() % 5 == 0 || coin.IsSpent()) {
                CTxOut txout;
                txout.nValue = CAmount(int64_t(insecure_rand()));
...
BOOST_AUTO_TEST_CASE(updatecoins_simulation_test) {
    ...
    for (int64_t i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            // 18/20 times reconnect previous or add a regular tx
            else {
                tx.nFlags = TX_FLAGS_NORMAL;
                COutPoint prevout;
                ...
                // 17/20 times create a regular tx
                else {
...
//    CScript scriptPubKey = GetScriptForDestination(CKeyID(uint160(ParseHex("816115944e077fe7c803cfa57f29b36bf87c1d35"))));
//    CTxOut txOut(60000000000, scriptPubKey);
//    Coin coin(txOut, 203998, false);
//    CDataStream stream(SER_DISK, CLIENT_VERSION);
//    stream << coin;
//    std::string hexStr = HexStr(stream);
//    std::cout<<hexStr<<std::endl;

    // Good example
    CDataStream ss1(
        ParseHex("97f23c83580000816115944e077fe7c803cfa57f29b36bf87c1d350000"),
        SER_DISK, CLIENT_VERSION);
    Coin c1;
    ss1 >> c1;
    BOOST_CHECK_EQUAL(c1.IsCoinBase(), false);
    BOOST_CHECK_EQUAL(c1.GetHeight(), 203998U);
    BOOST_CHECK_EQUAL(c1.GetTxOut().nValue, CAmount(60000000000LL));
...
static void SetCoinValue(const CAmount value, Coin &coin) {
    assert(value != ABSENT);
    coin.Clear();
    assert(coin.IsSpent());
    if (value != PRUNED) {
...
size_t InsertCoinMapEntry(CCoinsMap &map, const CAmount value, char flags) {
    if (value == ABSENT) {
...
void WriteCoinViewEntry(CCoinsView &view, const CAmount value, char flags) {
    CCoinsMap map;
...
public:
    SingleEntryCacheTest(const CAmount base_value, const CAmount cache_value,
                         char cache_flags) {
...
void CheckAccessCoin(const CAmount base_value, const CAmount cache_value,
                     const CAmount expected_value, char cache_flags,
                     char expected_flags) {
