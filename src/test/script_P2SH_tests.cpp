...
static bool Verify(const CScript &scriptSig, const CScript &scriptPubKey,
    ...
    txTo.vout[0].nValue = CAmount(1);

    return VerifyScript(
        scriptSig, scriptPubKey,
        (fStrict ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE) ,
        MutableTransactionSignatureChecker(&txTo, 0),
        &err);
}
...
BOOST_AUTO_TEST_CASE(sign) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        txTo[i].vout[0].nValue = CAmount(1);
...
BOOST_AUTO_TEST_CASE(AreInputsStandard) {
    ...
    txFrom.vout[0].nValue = CAmount(1000);
    // ordinary OP_CHECKSIG
    txFrom.vout[1].scriptPubKey = pay1;
    txFrom.vout[1].nValue = CAmount(2000);
    // ordinary OP_CHECKMULTISIG
    txFrom.vout[2].scriptPubKey = pay1of3;
    txFrom.vout[2].nValue = CAmount(3000);
    ...
    txFrom.vout[3].nValue = CAmount(4000);
    ...
    txFrom.vout[4].nValue = CAmount(5000);
    ...
    txFrom.vout[5].scriptPubKey =
        GetScriptForDestination(CScriptID(fifteenSigops));
    txFrom.vout[5].nValue = CAmount(5000);
    ...
    txFrom.vout[6].nValue = CAmount(6000);
    ...
    BOOST_CHECK(
        SignSignature(keystore, txFrom, txTo, 1, SIGHASH_ALL ));
    BOOST_CHECK(
        SignSignature(keystore, txFrom, txTo, 2, SIGHASH_ALL ));
    ...
    txToNonStd1.vout[0].scriptPubKey =
        GetScriptForDestination(key[1].GetPubKey().GetID());
    txToNonStd1.vout[0].nValue = CAmount(1000);
    ...
    txToNonStd2.vout[0].scriptPubKey =
        GetScriptForDestination(key[1].GetPubKey().GetID());
    txToNonStd2.vout[0].nValue = CAmount(1000);
