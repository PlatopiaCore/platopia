...
BOOST_AUTO_TEST_CASE(BlockPolicyEstimates) {
    CTxMemPool mpool(CFeeRate(CAmount(1000)));
    TestMemPoolEntryHelper entry;
    CAmount basefee(2000);
    CAmount deltaFee(1200);
    ...
    while (blocknum < 200) {
        ...
        if (blocknum == 30) {
            ...
            BOOST_CHECK(mpool.estimateFee(1) == CFeeRate(CAmount(0)));
            BOOST_CHECK(mpool.estimateFee(2) == CFeeRate(CAmount(0)));
            BOOST_CHECK(mpool.estimateFee(3) == CFeeRate(CAmount(0)));
    ...
    for (int i = 1; i < 10; i++) {
        ...
        if (i > 1) {
            ...
            BOOST_CHECK(origFeeEst[i - 1] == CFeeRate(CAmount(0)).GetFeePerK());
        }
    }
    ...
    int answerFound;
    for (int i = 1; i < 10; i++) {
        BOOST_CHECK(mpool.estimateFee(i) == CFeeRate(CAmount(0)) ||
                    mpool.estimateFee(i).GetFeePerK() >
                        origFeeEst[i - 1] - deltaFee);
        CAmount a1 = mpool.estimateSmartFee(i, &answerFound).GetFeePerK();
        CAmount a2 = origFeeEst[answerFound - 1] - deltaFee;
        BOOST_CHECK(a1 > a2);
    }
