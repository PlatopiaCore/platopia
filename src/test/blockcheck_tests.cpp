...
BOOST_AUTO_TEST_CASE(blockfail) {
    ...
    RunCheckOnBlock(config, block, "bad-cb-prevout-hash");
    ...
    auto headerSize = ::GetSerializeSize(block.GetBlockHeader(), SER_NETWORK, PROTOCOL_VERSION);
    auto maxTxCount = ((DEFAULT_MAX_BLOCK_SIZE - headerSize) / txSize);
    ...
    // But reject it with one more transaction as it goes oGetver the maximum
    // allowed block size.
    tx.vin[0].prevout.hash = GetRandHash();
    block.vtx.push_back(MakeTransactionRef(tx));
