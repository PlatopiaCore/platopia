...
BOOST_AUTO_TEST_CASE(get_next_work) {
    ...
    int64_t nLastRetargetTime = 1512403200;
    CBlockIndex pindexLast;
    pindexLast.nHeight = 961;
    pindexLast.nTime = nLastRetargetTime + 2 * 24 * 60 * 60;
...
BOOST_AUTO_TEST_CASE(retargeting_test) {
    ...
    for (size_t i = 100; i < 110; i++) {
        blocks[i] = GetBlockIndex(&blocks[i - 1], 2 * 540, initialBits);
        BOOST_CHECK_EQUAL(
    ...
    blocks[110] = GetBlockIndex(&blocks[109], 2 * 540, initialBits);
    currentPow.SetCompact(currentPow.GetCompact());
    currentPow += (currentPow >> 2);
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(&blocks[110], &blkHeaderDummy, config), 0x1c7fffff);

    // As we continue with 2h blocks, difficulty continue to decrease.
    blocks[111] =
        GetBlockIndex(&blocks[110], 2 * 540, currentPow.GetCompact());
    currentPow.SetCompact(currentPow.GetCompact());
    currentPow += (currentPow >> 2);
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(&blocks[111], &blkHeaderDummy, config), 0x1d009fff);

    // We decrease again.
    blocks[112] =
        GetBlockIndex(&blocks[111], 2 * 540, currentPow.GetCompact());
    currentPow.SetCompact(currentPow.GetCompact());
    currentPow += (currentPow >> 2);
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(&blocks[112], &blkHeaderDummy, config), 0x1d00c7fe);

    // We check that we do not go below the minimal difficulty.
    blocks[113] =
        GetBlockIndex(&blocks[112], 2 * 540, currentPow.GetCompact());
    currentPow.SetCompact(currentPow.GetCompact());
    currentPow += (currentPow >> 2);
    BOOST_CHECK(powLimit.GetCompact() != currentPow.GetCompact());
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(&blocks[113], &blkHeaderDummy, config), 0x1d00f9fd);

    // Once we reached the minimal difficulty, we stick with it.
    blocks[114] = GetBlockIndex(&blocks[113], 2 * 540, powLimit.GetCompact());
    BOOST_CHECK(powLimit.GetCompact() != currentPow.GetCompact());
    BOOST_CHECK_EQUAL(
        GetNextWorkRequired(&blocks[114], &blkHeaderDummy, config), powLimit.GetCompact());
}
...
BOOST_AUTO_TEST_CASE(cash_difficulty_test) {
    ...
    for (i = 1; i < 2050; i++) {
        blocks[i] = GetBlockIndex(&blocks[i - 1], 90, initialBits);
    }
    ...
    for (size_t j = 0; j < 10; i++, j++) {
        blocks[i] = GetBlockIndex(&blocks[i - 1], 90, nBits);
        BOOST_CHECK_EQUAL(
    ...
    blocks[i] = GetBlockIndex(&blocks[i - 1], 2 * 90 - 900, nBits);
    BOOST_CHECK_EQUAL(
        GetNextCashWorkRequired(&blocks[i++], &blkHeaderDummy, config), nBits);
    ...
    blocks[i] = GetBlockIndex(&blocks[i - 1], 80, nBits);
    BOOST_CHECK_EQUAL(
        GetNextCashWorkRequired(&blocks[i++], &blkHeaderDummy, config), nBits);
    ...
    BOOST_CHECK_EQUAL(nBits, 0x1c0fdf98);
    ...
    // Check the actual value.
    BOOST_CHECK_EQUAL(nBits, 0x1c0ddd2e);
    ...
    // Check the actual value.
    BOOST_CHECK_EQUAL(nBits, 0x1c0dc09b);
