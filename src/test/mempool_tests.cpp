...
BOOST_AUTO_TEST_CASE(MempoolRemoveTest) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        txParent.vout[i].nValue = CAmount(33000LL);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        txChild[i].vout[0].nValue = CAmount(11000LL);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        txGrandChild[i].vout[0].nValue = CAmount(11000LL);
    }

    CTxMemPool testPool(CFeeRate(CAmount(0)));
...
BOOST_AUTO_TEST_CASE(MempoolIndexingTest) {
    CTxMemPool pool(CFeeRate(CAmount(0)));
    TestMemPoolEntryHelper entry;
    ...
    pool.addUnchecked(tx1.GetId(),
                      entry.Fee(CAmount(10000LL)).Priority(10.0).FromTx(tx1));
    ...
    pool.addUnchecked(tx2.GetId(),
                      entry.Fee(CAmount(20000LL)).Priority(9.0).FromTx(tx2));
    ...
    pool.addUnchecked(tx3.GetId(),
                      entry.Fee(CAmount(0LL)).Priority(100.0).FromTx(tx3));
    ...
    pool.addUnchecked(tx4.GetId(),
                      entry.Fee(CAmount(15000LL)).Priority(1.0).FromTx(tx4));
    ...
    pool.addUnchecked(tx5.GetId(), entry.Fee(CAmount(10000LL)).FromTx(tx5));
    ...
    pool.addUnchecked(tx6.GetId(), entry.Fee(CAmount(0LL)).FromTx(tx6));
    ...
    std::string dummy;
    BOOST_CHECK_EQUAL(
        pool.CalculateMemPoolAncestors(entry.Fee(CAmount(2000000LL)).FromTx(tx7),
    ...
    pool.addUnchecked(tx8.GetId(), entry.Fee(CAmount(0LL)).Time(2).FromTx(tx8),
                      setAncestors);
    ...
    pool.addUnchecked(tx9.GetId(), entry.Fee(CAmount(0LL)).Time(3).FromTx(tx9),
                      setAncestors);
    ...
    BOOST_CHECK_EQUAL(pool.CalculateMemPoolAncestors(
                          entry.Fee(CAmount(200000LL)).Time(4).FromTx(tx10),
    ...
    sortedOrder.insert(sortedOrder.begin() + 5, tx8.GetId().ToString());
    sortedOrder.insert(sortedOrder.begin() + 6, tx9.GetId().ToString());
...
BOOST_AUTO_TEST_CASE(MempoolAncestorIndexingTest) {
    ...
    pool.addUnchecked(tx7.GetId(), entry.Fee(CAmount(fee)).FromTx(tx7));
    BOOST_CHECK_EQUAL(pool.size(), 7UL);
...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest) {
    CTxMemPool pool(CFeeRate(CAmount(1000)));
    TestMemPoolEntryHelper entry;
    entry.dPriority = 10.0;
    ...
    tx1.vout[0].nValue = 10 * COIN;
    pool.addUnchecked(tx1.GetId(),
                      entry.Fee(CAmount(10000LL)).FromTx(tx1, &pool));
    ...
    pool.addUnchecked(tx2.GetId(),
                      entry.Fee(CAmount(5000LL)).FromTx(tx2, &pool));
    ...
    pool.addUnchecked(tx3.GetId(),
                      entry.Fee(CAmount(20000LL)).FromTx(tx3, &pool));
    ...
    CFeeRate maxFeeRateRemoved(CAmount(25000), GetTransactionSize(tx3) +
                                                  GetTransactionSize(tx2));
    BOOST_CHECK_EQUAL(pool.GetMinFee(1).GetFeePerK(),
                      maxFeeRateRemoved.GetFeePerK() + CAmount(1000));
    ...
    pool.addUnchecked(tx4.GetId(),
                      entry.Fee(CAmount(7000LL)).FromTx(tx4, &pool));
    pool.addUnchecked(tx5.GetId(),
                      entry.Fee(CAmount(1000LL)).FromTx(tx5, &pool));
    pool.addUnchecked(tx6.GetId(),
                      entry.Fee(CAmount(1100LL)).FromTx(tx6, &pool));
    pool.addUnchecked(tx7.GetId(),
                      entry.Fee(CAmount(9000LL)).FromTx(tx7, &pool));
    ...
    BOOST_CHECK_EQUAL(pool.GetMinFee(1).GetFeePerK(),
                      (maxFeeRateRemoved.GetFeePerK() + CAmount(1000)) / 2);
    ...
    BOOST_CHECK_EQUAL(
        ...
        (maxFeeRateRemoved.GetFeePerK() + CAmount(1000)) / 4);
    ...
    BOOST_CHECK_EQUAL(
        ...
        (maxFeeRateRemoved.GetFeePerK() + CAmount(1000)) / 8);
    // ... with a 1/4 halflife when mempool is < 1/4 its target size

    SetMockTime(42 + 7 * CTxMemPool::ROLLING_FEE_HALFLIFE +
                CTxMemPool::ROLLING_FEE_HALFLIFE / 2 +
                CTxMemPool::ROLLING_FEE_HALFLIFE / 4);
    BOOST_CHECK_EQUAL(pool.GetMinFee(1).GetFeePerK(), CAmount(1000));
