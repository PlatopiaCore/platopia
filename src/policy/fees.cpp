...
CBlockPolicyEstimator::CBlockPolicyEstimator(const CFeeRate &_minRelayFee)
    ...
    static_assert(MIN_FEERATE > CAmount(0), "Min feerate must be nonzero");
    ...
    for (double bucketBoundary = minTrackedFee.GetFeePerK();
         bucketBoundary <= double(MAX_FEERATE);
         bucketBoundary *= FEE_SPACING) {
        vfeelist.push_back(bucketBoundary);
    }

    vfeelist.push_back(double(INF_FEERATE));
...
CFeeRate CBlockPolicyEstimator::estimateFee(int confTarget) {
    ...
    return CFeeRate(CAmount(int64_t(median)));
}
...
CFeeRate CBlockPolicyEstimator::estimateSmartFee(int confTarget,
    ...
    CAmount minPoolFee =
        pool.GetMinFee(GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) *
                       1000000)
            .GetFeePerK();
    if (minPoolFee > CAmount(0) && minPoolFee > CAmount(int64_t(median))) {
...
double CBlockPolicyEstimator::estimateSmartPriority(int confTarget,
    ...
    CAmount minPoolFee =
        pool.GetMinFee(GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) *
                       1000000)
            .GetFeePerK();
    if (minPoolFee > CAmount(0)) {
        return double(INF_PRIORITY);
    }
...
FeeFilterRounder::FeeFilterRounder(const CFeeRate &minIncrementalFee) {
    ...
    feeset.insert(CAmount(0));
    for (double bucketBoundary = minFeeLimit;
         bucketBoundary <= double(MAX_FEERATE);
         bucketBoundary *= FEE_SPACING) {
        feeset.insert(CAmount(int64_t(bucketBoundary)));
    }
}
