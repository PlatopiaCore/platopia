...
void RemoveAddress(const CTxDestination &dest, const std::string &strLabel);
...
UniValue importprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    CKeyID vchAddress = pubkey.GetID();
    {
        ...
        if (pwalletMain->HaveWatchOnly(GetScriptForRawPubKey(pubkey)))
            pwalletMain->RemoveWatchOnly(GetScriptForRawPubKey(pubkey));
...
void RemoveScript(const CScript &script, const std::string &strLabel,
                  bool isRedeemScript) {
//    if (!isRedeemScript && ::IsMine(*pwalletMain, script) == ISMINE_SPENDABLE)
    if (isRedeemScript)
        throw JSONRPCError(RPC_WALLET_ERROR, "The wallet already contains the "
    ...
    if (pwalletMain->HaveWatchOnly(script) && pwalletMain->RemoveWatchOnly(script))
        throw JSONRPCError(RPC_WALLET_ERROR, "Error removing address  wallet");
...
void RemoveAddress(const CTxDestination &dest, const std::string &strLabel) {
    CScript script = GetScriptForDestination(dest);
    RemoveScript(script, strLabel, false);
    // add to address book or update label
    if (IsValidDestination(dest))
        pwalletMain->DelAddressBook(dest);
}
...
UniValue importaddress(const Config &config, const JSONRPCRequest &request) {
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           "Invalid Platopia address or script");
    }
...
UniValue removeaddress(const Config &config, const JSONRPCRequest &request) {
    if (!EnsureWalletIsAvailable(request.fHelp)) return NullUniValue;

    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "importaddress \"address\" ( \"label\" )\n"
            "\nRemove a script or address that can be watched as if it "
            "were in your wallet but cannot be used to spend.\n"
            "\nArguments:\n"
            "1. \"script\"           (string, required) The hex-encoded script "
            "(or address)\n"
            ...
            "optional label\n"
            "the wallet for transactions\n"
            "3. p2sh                 (boolean, optional, default=false) Add "
            "the P2SH version of the script as well\n"

            "\nNote: If you remove a non-standard raw script in hex form, "
            "outputs sending to it will be treated\n"
            "as change, and not show up in many RPCs.\n"
            "\nExamples:\n"
            "\nRemove a script" +
    ...
    // Whether to import a p2sh version, too
    bool fP2SH = false;
    if (request.params.size() > 2) fP2SH = request.params[2].get_bool();
    ...
    if (IsValidDestination(dest)) {
        ...
        RemoveAddress(dest, strLabel);
    } else if (IsHex(request.params[0].get_str())) {
        std::vector<uint8_t> data(ParseHex(request.params[0].get_str()));
        RemoveScript(CScript(data.begin(), data.end()), strLabel, fP2SH);
    } else {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           "Invalid Platopia address or script");
    }
...
UniValue dumpprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The platopia address for the "
            "private key\n"
            "\nResult:\n"
...
static const CRPCCommand commands[] = {
    ...
    { "wallet",             "removeaddress",            removeaddress,            true,   {"address","label"} },
