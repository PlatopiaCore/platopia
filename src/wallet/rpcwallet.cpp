...
static UniValue getnewaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            ...
            "\nReturns a new Platopia address for receiving payments.\n"
            ...
            "the given name.\n"
            "\nResult:\n"
            "\"address\"    (string) The new platopia address\n"
            "\nExamples:\n" +
...
static UniValue getaccountaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current Platopia address for receiving "
            ...
            "the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account platopia address\n"
            "\nExamples:\n" +
...
static UniValue getrawchangeaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Platopia address, for receiving change.\n"
...
static UniValue setaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The platopia address to "
...
static UniValue getaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The platopia address for "
            "account lookup.\n"
            "\nResult:\n"
...
static UniValue getaddressesbyaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a platopia address associated with "
            "the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
static void SendMoney(const CTxDestination &address, const CAmount nValue,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase() + "\nArguments:\n"
                                        "1. \"address\"            (string, "
                                        "required) The platopia address to send "
                                        "to.\n"
                                        "2. \"amount\"             (numeric or "
...
static UniValue listaddressgroupings(const Config &config,
    ...
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The platopia address\n"
...
static UniValue getreceivedbyaddress(const Config &config,
    ...
    if (!IsMine(*pwalletMain, scriptPubKey)) {
        return ValueFromAmount(CAmount(0));
    }
...
static UniValue sendfrom(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 3 ||
        ...
        throw std::runtime_error(
            ...
            "reflect\n"
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The platopia address "
            "to send funds to.\n"
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
    ...
    for (const std::string &name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Platopia address: ") +
                                   name_);
        }
...
static UniValue addmultisigaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "Each key is a Platopia address or hex-encoded public key.\n"
            ...
            "     [\n"
            "       \"address\"  (string) platopia address or hex-encoded "
            "public key\n"
            "       ...,\n"
            "     ]\n"
...
void ListTransactions(const CWalletTx &wtx, const std::string &strAccount,
    ...
    // Sent
    if ((!listSent.empty() || nFee != CAmount(0)) &&
...
static UniValue listtransactions(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The platopia address of "
...
static UniValue listaccounts(const Config &config,
    ...
    for (const std::pair<CTxDestination, CAddressBookData> &entry :
        ...
        if (IsMine(*pwalletMain, entry.first) & includeWatchonly) {
            mapAccountBalances[entry.second.name] = CAmount(0);
        }
    }
...
static UniValue gettransaction(const Config &config,
    ...
    CAmount nFee =
        (wtx.IsFromMe(filter) ? wtx.tx->GetValueOutWithoutInterest() - nDebit : CAmount(0));
...
static UniValue lockunspent(const Config &config,
    ...
    for (size_t idx = 0; idx < outputs.size(); idx++) {
        ...
        COutPoint outpt(uint256S(txid), nOutput, 0);
...
static UniValue settxfee(const Config &config, const JSONRPCRequest &request) {
    ...
    payTxFee = CFeeRate(nAmount, 1000*100);  // Update from 1000 PerKB to 100 SatoshiPerB
    return true;
}
...
static UniValue listunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "addresses.\n"
            "\nArguments:\n"
            "1. includeLocked    (bool, optional, default=false) Whether include locked utxo\n"
            "2. minconf          (numeric, optional, default=1) The minimum "
            "confirmations to filter\n"
            "3. maxconf          (numeric, optional, default=9999999) The "
            "maximum confirmations to filter\n"
            "4. \"addresses\"    (string) A json array of platopia addresses to "
            "filter\n"
            "    [\n"
            "      \"address\"   (string) platopia address\n"
            "      ,...\n"
            "    ]\n"
            "5. include_unsafe (bool, optional, default=true) Include outputs "
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the platopia address\n"
    ...
    if (request.params.size() > 0 && !request.params[0].isNull()) {
        ...
        includeLocked = request.params[0].getBool();
    }
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    pwalletMain->AvailableCoins(vecOutputs, !include_unsafe, nullptr, true, includeLocked);
...
static UniValue fundrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address) The platopia address to receive the change\n"
    ...
    CFeeRate feeRate = CFeeRate(CAmount(0));
            ...
            if (options.exists("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(
                        ...
                        "changeAddress must be a valid platopia address");
                }
...
static UniValue deposittoaddress(const Config &config,
                                  const JSONRPCRequest &request) {
    if (!EnsureWalletIsAvailable(request.fHelp)) {
        return NullUniValue;
    }
    ...
    if (request.fHelp || request.params.size() < 2 ||
        request.params.size() > 3) {
        throw std::runtime_error(
            "deposittoaddress locktime amount ( \"address\" )\n"
            "\nDeposit an amount to a given address.\n" +
            HelpRequiringPassphrase() + "\nArguments:\n"
                                        "1. \"lockdays\"            (numeric, "
                                        "required) The lockdays to lock "
                                        "to.\n"
                                        "2. \"amount\"             (numeric or "
                                        "string, required) The amount in " +
                                        CURRENCY_UNIT +
                                        ...
                                        "The platopia address to deposit to \n"

                                        "\nResult:\n"
                                        "\"txid\"                  (string) The transaction id.\n"
                                        "\nExamples:\n" +
            HelpExampleCli("deposittoaddress",
                           "16, 0.1") +
            HelpExampleCli("deposittoaddress", "160, 0.1, \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\"") +
            HelpExampleCli(
                "deposittoaddress",
                "160, 0.1, \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\"") +
            HelpExampleRpc("deposittoaddress", "160, 0.1, \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\""));
    }
    ...
    uint32_t nLockBlocks = 1;
    int nLockDays = 0;
    RPCTypeCheckArgument(request.params[0], UniValue::VNUM);
    nLockDays = request.params[0].get_int();


    if (nLockDays < 16 ) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Invalid locktime, must > 16 blockdays"));
    }
    nLockBlocks = nLockDays * Params().GetConsensus().nBlocksPerDay;
    ...
    CRecipient recipient = {scriptPubKey, nAmount, nLockBlocks, false};
    vecSend.push_back(recipient);
...
static const CRPCCommand commands[] = {
    ...
    { "interest",           "deposittoaddress",         deposittoaddress,         false,  {"lockdays", "principal", "address"} }
};
// clang-format on
