...
struct CompareValueOnly {
    bool operator()(
        const std::pair<CAmount, std::pair<const CWalletTx *, unsigned int>> &t1,
...
bool CWallet::AccountMove(std::string strFrom, std::string strTo,
                          const CAmount nAmount, std::string strComment) {
    CWalletDB walletdb(strWalletFile);
...
bool CWallet::IsFromMe(const CTransaction &tx) const {
    return GetDebit(tx, ISMINE_ALL) > CAmount(0);
}
...
void CWalletTx::GetAccountAmounts(const std::string &strAccount,
                                  ...
                                  CAmount &nFee,
                                  const isminefilter &filter) const {
    nReceived = nSent = nFee = CAmount(0);
...
CAmount CWalletTx::GetDebit(const isminefilter &filter) const {
    if (tx->vin.empty()) return CAmount(0);
    ...
    if (filter & ISMINE_WATCH_ONLY) {
        if (fWatchDebitCached) {
            ...
            debit += CAmount(nWatchDebitCached);
        }
    }
...
void CWallet::AvailableCoins(std::vector<COutput> &vCoins, bool fOnlyConfirmed,
                             ...
                             bool fIncludeZeroValue, bool includeLocked) const {
    vCoins.clear();
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
        ...
        if (!includeLocked && pcoin->IsCoinBase() && pcoin->GetBlocksToMaturity() > 0) {
            continue;
        }
        ...
        // Platopia-Core: Removed check that prevents consideration of coins from
        ...
        // Platopia-Core: retained this check as 'replaced_by_txid' is still set
        // in the wallet code.
        ...
        const int confirmations = pcoin->GetDepthInMainChain();
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            isminetype mine = IsMine(pcoin->tx->vout[i]);
            if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                !IsLockedCoin((*it).first, i) &&
				(includeLocked || confirmations > pcoin->tx->vout[i].nLockTime) &&
                (pcoin->tx->vout[i].nValue > CAmount(0) || fIncludeZeroValue) &&
...
void CWallet::GetAllDeposit(std::vector<CTxOutVerbose> &vDepositItem) const {
    vDepositItem.clear();
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
        ...
        const int confirmations = pcoin->GetDepthInMainChain();
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            if(pcoin->tx->vout[i].nPrincipal <= 0)
            {
                continue;
            }
            if (IsMine(pcoin->tx->vout[i]) != ISMINE_NO) {
                vDepositItem.push_back(CTxOutVerbose(pcoin->tx->vout[i], pcoin->tx->GetId(), i, pcoin->GetTxHeight()));
            }
        }
    }
}
...
bool CWallet::FundTransaction(CMutableTransaction &tx, CAmount &nFeeRet,
    ...
    for (size_t idx = 0; idx < tx.vout.size(); idx++) {
        ...
        CRecipient recipient = {txOut.scriptPubKey, txOut.nValue, 0,
...
bool CWallet::CreateTransaction(const std::vector<CRecipient> &vecSend,
                                ...
                                const CCoinControl *coinControl, bool sign, bool isDeposit) {
    CAmount nValue(0);
    int nChangePosRequest = nChangePosInOut;
    unsigned int nSubtractFeeFromAmount = 0;
    for (const auto &recipient : vecSend) {
        if (nValue < CAmount(0) || recipient.nAmount < CAmount(0)) {
...

    {
        ...
        while (true) {
            ...
            // vouts to the payees
            for (const auto &recipient : vecSend) {
                CTxOut txout(recipient.nAmount, recipient.scriptPubKey, "", recipient.nLockTime);
                ...
                CAmount nInterest = 0;
                if (isDeposit) {
                    if (recipient.nLockTime >= 16 * Params().GetConsensus().nBlocksPerDay)
                        nInterest = GetInterest(txout.nValue, recipient.nLockTime, chainActive.Height());
                    txout.nPrincipal = txout.nValue;
                    txout.nValue += nInterest;
                }
            ...
            for (const auto &coin : setCoins) {
                txNew.vin.push_back(
                    CTxIn(coin.first->GetId(), coin.second, coin.first->tx->vout[coin.second].nValue, CScript()));
            }
            ...
            if (coinControl && nFeeNeeded > CAmount(0) &&
    ...
    if (GetBoolArg("-walletrejectlongchains",
        ...
        LockPoints lp;
        CTxMemPoolEntry entry(wtxNew.tx, CAmount(0), 0, 0, 0, CAmount(0), false,
                              0, lp);
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances() {
    ...
    for (std::pair<uint256, CWalletTx> walletEntry : mapWallet) {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            if (!balances.count(addr)) balances[addr] = CAmount(0);
            balances[addr] += n;
        }
    }
...
CAmount CWallet::GetAccountBalance(CWalletDB &walletdb,
    ...
    for (std::map<uint256, CWalletTx>::iterator it = mapWallet.begin();
        ...
        LogPrintf("Received: %d, Sent: %d, Fee: %d, Tx: %s\n", nReceived, nSent, nFee, wtx.tx->GetHash().GetHex());
        if (nReceived != CAmount(0) && wtx.GetDepthInMainChain() >= nMinDepth) {
            nBalance += nReceived;
        }
...
int CMerkleTx::GetTxHeight() const
{
    if (hashUnset()) {
        return 0;
    }
