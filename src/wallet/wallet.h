...
//! -paytxfee default
static const CAmount DEFAULT_TRANSACTION_FEE(1000 * 100);
//! -fallbackfee default
static const CAmount DEFAULT_FALLBACK_FEE(2000000);
//! -mintxfee default
static const CAmount DEFAULT_TRANSACTION_MINFEE(1000 * 100);
...
public:
    ...
    /*
     * Return height of transaction in blockchain:
     * -1 not included in chain
     * >=0 height in chain
     */
...
public:
    ...
    void Init(const CWallet *pwalletIn) {
        ...
        fChangeCached = false;
        nDebitCached = CAmount(0);
        nCreditCached = CAmount(0);
        nImmatureCreditCached = CAmount(0);
        nAvailableCreditCached = CAmount(0);
        nWatchDebitCached = CAmount(0);
        nWatchCreditCached = CAmount(0);
        nAvailableWatchCreditCached = CAmount(0);
        nImmatureWatchCreditCached = CAmount(0);
    ...
    bool IsFromMe(const isminefilter &filter) const {
        return (GetDebit(filter) > CAmount(0));
    }
...
public:
    ...
    void AvailableCoins(std::vector<COutput> &vCoins,
                        ...
                        bool includeLocked = false) const;

    void GetAllDeposit(std::vector<CTxOutVerbose> &vDepositItem) const;
    ...
    bool CreateTransaction(const std::vector<CRecipient> &vecSend,
                           ...
                           const CCoinControl *coinControl = nullptr,
                           bool sign = true, bool isDeposit = false);
    ...
    CAmount GetAccountBalance(CWalletDB &walletdb, const std::string &strAccount,
