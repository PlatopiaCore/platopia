...
BOOST_AUTO_TEST_CASE(coin_selection_tests) {
    ...
    for (int i = 0; i < RUN_TESTS; i++) {
        ...
        for (CAmount amt = CAmount(1500); amt < COIN; amt = 10 * amt) {
            empty_wallet();
            ...
            BOOST_CHECK(wallet.SelectCoinsMinConf(CAmount(2000), 1, 1, 0, vCoins,
                                                  setCoinsRet, nValueRet));
            if (amt - CAmount(2000) < MIN_CHANGE) {
                // needs more than one input:
                uint16_t returnSize = std::ceil(
                    (2000.0 + MIN_CHANGE) / amt);
                CAmount returnValue = returnSize * amt;
...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain100Setup) {
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetImmatureBalance(), 1842.3288 * COIN);
    }
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetImmatureBalance(), 921.1644 * COIN);
    }
