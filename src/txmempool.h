...
public:
    ...
    const CAmount GetInterest() const {return nInterest;}
...
public:
    ...
    bool UseDescendantScore(const CTxMemPoolEntry &a) {
        double f1 = double(a.GetSizeWithDescendants() *
                           a.GetModifiedFee());
        double f2 =
            double(a.GetTxSize() * a.GetModFeesWithDescendants());
        return f2 > f1;
    }
};
...
public:
    bool operator()(const CTxMemPoolEntry &a, const CTxMemPoolEntry &b) {
        double f1 = double(b.GetTxSize() * a.GetModifiedFee());
        double f2 = double(a.GetTxSize() * b.GetModifiedFee());
        if (f1 == f2) {
