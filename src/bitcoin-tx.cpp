...
static void MutateTxAddInput(CMutableTransaction &tx,
    ...
    CTxIn txin(txid, vout, 0, CScript());
...
static CAmount AmountFromValue(const UniValue &value) {
    if (!value.isNum() && !value.isStr()) {
        throw std::runtime_error("CAmount is not a number or string");
    }
    ...
    CAmount amount = CAmount(n);

    if (!MoneyRange(amount)) {
        throw std::runtime_error("CAmount out of range");
    }
