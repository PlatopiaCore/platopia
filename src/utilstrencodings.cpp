...
ethash_h256_t ethash_h256_decode(const std::string &str)
{
    std::vector<unsigned char> vBytes = ParseHex(str.c_str() + 2);
    ethash_h256_t result;
    for (int i = 0; i < 32; i++) {
        result.b[31 - i] = (uint8_t)(vBytes.at(i));
    }
    return result;
}

ethash_h256_t ethash_h256_decode_big(const std::string &str)
{
    std::vector<unsigned char> vBytes = ParseHex(str.c_str() + 2);
    ethash_h256_t result;
    for (int i = 0; i < 32; i++) {
        result.b[i] = (uint8_t)(vBytes.at(i));
    }
    return result;
}

std::string ethash_h256_encode(const ethash_h256_t &hash)
{
    return ReverseHexStr(BEGIN(hash.b), END(hash.b));
}

std::string ethash_h256_encode_big(const ethash_h256_t &hash)
{
    return HexStr(BEGIN(hash.b), END(hash.b));
}
