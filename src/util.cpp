...
const char *const BITCOIN_CONF_FILENAME = "platopia.conf";
const char *const BITCOIN_PID_FILENAME = "platopia.pid";
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Platopia
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Platopia
// Mac: ~/Library/Application Support/Bitcoin
// Unix: ~/.bitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Platopia";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Platopia";
#else
    // Unix
    return pathRet / ".platopia";
#endif
#endif
}
...
void ShrinkDebugFile() {
    // CAmount of debug.log to save at end when shrinking (must fit in memory)
...
std::string CopyrightHolders(const std::string &strPrefix) {
    ...
    // Check for untranslated substitution to make sure Platopia Core copyright
    // is not removed by accident.
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION)
            .find("Platopia Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Platopia Core developers";
    }
