...
CTxMemPoolEntry::CTxMemPoolEntry(const CTransactionRef &_tx, const CAmount _nFee,
    ...
    nInterest = tx->GetInterest();
    nUsageSize = RecursiveDynamicUsage(*tx) + memusage::DynamicUsage(tx);

    nCountWithDescendants = 1;
    nSizeWithDescendants = GetTxSize();
    nModFeesWithDescendants = nFee;
    CAmount nValueIn = tx->GetValueOutWithoutInterest() + nFee;
...
void CTxMemPool::UpdateTransactionsFromBlock(
    ...
    for (const uint256 &hash : boost::adaptors::reverse(vHashesToUpdate)) {
        ...
        auto iter = mapNextTx.lower_bound(COutPoint(hash, 0, 0));
...
void CTxMemPool::removeRecursive(const CTransaction &origTx,
    ...
    if (origit != mapTx.end()) {
        ...
        for (size_t i = 0; i < origTx.vout.size(); i++) {
            auto it = mapNextTx.find(COutPoint(origTx.GetId(), i, origTx.vout[i].nValue));
            if (it == mapNextTx.end()) {
                continue;
            }
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const {
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin();
        ...
        auto iter = mapNextTx.lower_bound(COutPoint(it->GetTx().GetId(), 0, it->GetTx().vout[0].nValue));
...
CFeeRate CTxMemPool::GetMinFee(size_t sizelimit) const {
    ...
    if (!blockSinceLastRollingFeeBump || rollingMinimumFeeRate == 0) {
        return CFeeRate(CAmount(int64_t(rollingMinimumFeeRate)));
    }
    ...
    return std::max(CFeeRate(CAmount(int64_t(rollingMinimumFeeRate))),
                    incrementalRelayFee);
}
